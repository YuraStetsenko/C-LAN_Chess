...
namespace myChess {
	...
	struct ChessBoardFlags
	{
		...
		std::pair<unsigned, unsigned> enPassantTo = {0,0};
		unsigned halfMoveClock = 0;
		unsigned fullMoveCount = 0;
	};

	class ChessBoard {
	public:
		const ChessBoardFlags& crefFlags = flags;
	...
	private:
		...
		void updateEnPassant() {
			...
			{
				flags.enPassantTo = { 0,0 };
				return;
			}

			flags.enPassantTo = { i_to - ((((int)i_to) - ((int)i_from)) / 2), j_from };
			...
			for (short offset = -1; offset <= 1; offset += 2)
				...
				{
					playBoard[i_to][j_to + offset]->getPotentialMoves().insert(flags.enPassantTo);
				}
		...
		void updateCastlingMoves() {
			...
			const auto& king = flags.isWhitesTurn ? white_king : black_king;
			const auto& isUnderCheck = flags.isWhitesTurn ? flags.whiteUnderCheck : flags.blackUnderCheck;
			const auto& canCastleKingSide = flags.isWhitesTurn ? flags.canWhiteCastleKingSide : flags.canBlackCastleKingSide;
			const auto& canCastleQueenSide = flags.isWhitesTurn ? flags.canWhiteCastleQueenSide : flags.canBlackCastleQueenSide;
			const auto& attackedByEnemyCells = flags.isWhitesTurn ? attackedByBlackCells : attackedByWhiteCells;
			const auto& i = king.first, & j = king.second;
			...
			if (!isUnderCheck)
			{
				if (
					...
					!playBoard[i][j-1] &&
					!playBoard[i][j-2] &&
					!attackedByEnemyCells.contains({ i,j-1 }) &&
					!attackedByEnemyCells.contains({ i,j-2 })
				   )
					playBoard[i][j]->getLegalMoves().insert({ i,1 });
				...
				if (
					...
					!attackedByEnemyCells.contains({ i,j+1 }) &&
					!attackedByEnemyCells.contains({ i,j+2 }) &&
					!attackedByEnemyCells.contains({ i,j+3 })
				   )
					playBoard[i][j]->getLegalMoves().insert({ i,5 });
			}
		...
		void updateChecks()
		{
			...
			if (attackedByEnemyCells.contains(king))
			{
		...
		void updateFlags()
		{
			...
			const auto& lastMove = moves.top();

			flags.isWhitesTurn = !flags.isWhitesTurn;

			if ((lastMove.first.ptTo != EmptyCell && lastMove.first.ptTo != Castling) || lastMove.first.ptFrom == Pawn)
				flags.halfMoveClock = 0;

			if (flags.isWhitesTurn)
				++flags.fullMoveCount;
	...
	public:
		...
		void setNextPromotionType(const PieceType& type) {
			flags.ptPromotionTo = type;
		}
